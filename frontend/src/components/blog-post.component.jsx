...
const BLogPostCard = ({ content, author }) => {
    let { publishedAt, tags, title, des, banner, activity: { total_likes }, blog_id } = content;
    ...
    return (
        <Link to={`/blog/${blog_id}`} className="flex gap-8 items-center border-b border-grey pb-5 mb-4">
            <div className="w-full">
                <div className="flex gap-2 items-center mb-7">
                    ...
                    <p className="line-clamp-1">
                        {fullname} @{username} • {getDay(publishedAt)}
                    </p>
                </div>
            ...
            <div className="h-28 aspect-square bg-grey">
                <img src={banner} className="w-full h-full aspect-square object-cover" alt="blog banner" />
            </div>
        </Link>
    );
};
