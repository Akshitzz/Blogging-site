                ...
                Object.keys(stats).map((key, i) => {
                    return !key.includes("parent") ? 
                        <div
                            ...
                            >
                            <h1 className="text-xl lg:text-2xl mb-2">{(stats[key] || 0).toLocaleString()}</h1>
                            <p className="max-lg:text-dark-grey capitalize">{key.split("-")[i] || key}</p>
                        </div> 
                    : null
                        })
                    }
...
export const ManagePublishBlogCard = ({blog}) => {
    ...
    return (
       ...
       <div className="flex gap-10 border-b mb-6 max-md:px-4 border-grey pb-6 items-center">
                <img src={banner} className="max-md:hidden lg:hidden xl:block w-28 h-28 bg-grey object-cover" alt="blog banner"/>
                <div className="flex flex-col justify-between py-2 w-full min-w-[300px]">
                        ...
                        <button className="pr-4 py-2 underline">
                            Delete
                        </button>
                            </div>
                </div>
...
const deleteBlog = (blog, access_token, target) => {
    ...
    .then(() => {
        ...
        setStateFunc(prevVal => {
            let {deleteDocCount = 0, totalDocs, results} = prevVal;
